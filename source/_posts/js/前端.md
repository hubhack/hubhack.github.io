...
超文本就是超出纯文本的范畴, 例如描述文本的颜色、大小、字体等信息、 或使用图片音频、 视屏等非文本内容.
...
client客户端向server发起请求 通过资源定位符, 找到映射文件, server端读取文件后, 一个个字节又通过管道发回到客户端, 客户端收到之后save下来, 打开html 文本流就是字节流(区别是字节流没有编码) .
